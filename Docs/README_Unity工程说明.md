...
### 最近工程变更（2026-01-29）
- **UITheme**：`DefaultFont` 改为静态缓存并保证永不返回 null（LegacyRuntime → Arial → CreateDynamicFontFromOSFont），避免设置等弹窗内 Text 配置时 NullReferenceException。
- **SettingsPopup**：CreateToggleButton/CreateSafeArea/CreateText/CreateButton 增加 parent 等 null 判断；Start 在 safe 为 null 时 return；设置弹窗（主菜单或暂停进入）可正常打开。
- **SettlementPanel**：新增 OnDestroy，移除 LEVEL_COMPLETED/LEVEL_FAILED 监听，避免面板销毁后仍被事件调用。
- **GameplayPanel**：仅在进行中或暂停时刷新血条/金币等（IsGameplayActive）；进入结算后战斗界面不再更新；右上角金币与按钮间距加大，减少重叠。
